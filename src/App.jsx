...
text: “#2a2520”, textSub: “#7a7268”, textMuted: “#b0a898”,
green: “#2d6a4f”, greenLight: “#e8f5ee”, blue: “#2563a8”, blueLight: “#e8f0fb”,
red: “#c0392b”, purple: “#6b46c1”,
userBubble: “#2d4a3e”, userText: “#e8f5ee”,
};
...
opts: [“独身・一人暮らし”, “独身・実家”, “既婚・子なし”, “既婚・子育て中（乳幼児）”, “既婚・子育て中（小中高）”, “シングルペアレント”, “介護中”, “育休・産休復帰直後”],
},
personality: {
label: “性格・タイプ”, icon: “🧠”, multi: true,
opts: [
“真面目・完璧主義”, “明るいが詰めが甘い”, “内向的・慎重派”,
“反発しやすい・自己主張強め”, “やる気が見えない”, “優秀だが孤立しがち”,
“感情的になりやすい”, “素直だが自信がない”, “受け身・指示待ち”,
“プライドが高い・負けず嫌い”, “表面上は元気だが無理している”,
“チームより個人プレー優先”, “空気を読みすぎて本音を言えない”,
“コミュ力は高いが継続力が弱い”, “仕事に意味・意義を求める”,
“変化を嫌い現状維持志向”, “完璧にできるまで報告しない”,
“批判に過剰に傷つく”, “承認欲求がとても強い”, “自己犠牲型・断れない”,
],
},
issue: {
label: “最近の状況・悩み”, icon: “⚡”, multi: true,
opts: [
“モチベが急落している”, “ミスが増えている”, “転職を考えていそう”,
“残業・疲弊が続いている”, “人間関係でトラブル”, “昇進・キャリアで悩んでいる”,
“プライベートで問題がある”, “リモートで孤立しがち”, “育休・産休復帰後で不安定”,
“メンタル不調の兆候がある”, “チームの雰囲気が悪化している”, “目標が見えず停滞”,
“急な異動・配置転換で戸惑い”, “特に問題はなさそう”,
“業務量過多で品質が落ちている”, “スキルアップへの焦り”,
“上司との関係に距離感”, “同期との差に焦りを感じている”,
“会社の方針・文化への不満”, “評価制度への不満・不信感”,
],
},
};
...
if (userMsgs.some(m => m.length < 8)) s -= 5;
s = Math.max(25, Math.min(100, s));

const grades = [
{ min: 90, grade: “S”, label: “卓越した1on1力”, color: C.red, comments: [“🏆 S評価達成！あなたは部下が自然に話せる最高の聴き手です！今日の1on1、部下の心に確実に届きました。”, “✨ 完璧です！この傾聴力と問いかけ、チームの未来を変えます。あなたのチームは最高の関係の質を持っています！”, “🔥 素晴らしい！成功循環モデルの「関係の質」を完璧に実践できています。これがチームの好循環を生みます！”] },
...
{ min: 68, grade: “A”, label: “良好・確かな実力”, color: C.green, comments: [“👍 A評価！傾聴の姿勢はできています。自信を持って！この調子で続けると半年後のチームが変わります。”, “💡 いい1on1でした。あなたは部下が「話しやすい上司」です。それだけで信頼の土台ができています。”, “📈 着実に力がついています！次はリフレクション（「それはどういうこと？」）を意識してみて。”] },
...
return `【ペルソナ】 あなた: ${userRole === "exec" ? "役員" : userRole === "senior" ? "シニアマネージャー" : "マネージャー"}（${userAge}歳） 部下: ${p.ageLabel} / ${p.industry} / ${p.job} / ${p.position}（${p.ageRelation}） 家族: ${p.family} 性格: ${p.personality.join("・")} 状況: ${p.issue.join("・")} ${ageNote ? `\n${ageNote}`: ""}${roleNote ?`\n${roleNote}` : “”}
...
const [screen, setScreen] = useState(savedProfile ? “persona” : “home”);
...
const rec = { date: new Date().toLocaleDateString(“ja-JP”), persona: `${subAgeLabel}/${fp.job}/${fp.position}`, grade: score.grade, score: score.score, personality: fp.personality[0] || “”, issue: fp.issue[0] || “” };
...
<style>{`*{box-sizing:border-box;margin:0;padding:0;} @keyframes fadeUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}} @keyframes pulse{0%,100%{opacity:0.3;transform:scale(0.7)}50%{opacity:1;transform:scale(1)}} ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#ddd} textarea:focus{border-color:${C.borderActive}!important;outline:none;} input[type=range]{-webkit-appearance:none;height:4px;border-radius:2px;background:#e0dbd2;} input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:20px;height:20px;border-radius:50%;background:${C.gold};cursor:pointer;}`}</style>
...
```
  ...
  <div style={{ padding: "12px 16px 0", borderBottom: `1px solid ${C.border}`, background: C.surface, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexShrink: 0 }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
      ...
      <button onClick={() => setScreen("home")} style={{ padding: "4px 8px", background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 10, color: C.textSub, cursor: "pointer" }}>⚙️ 設定</button>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 32px" }}>
      ...
      {PERSONA_KEYS.map(k => (
        <div key={k} style={{ marginBottom: 18 }}>
          <div style={{ fontSize: 12, color: C.textSub, marginBottom: 8, display: "flex", alignItems: "center", gap: 6, fontWeight: 600 }}>
            {PERSONA[k].icon} {PERSONA[k].label}
            {PERSONA[k].multi && <span style={{ fontSize: 10, color: C.textMuted, fontWeight: 400 }}>複数選択可</span>}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {PERSONA[k].opts.map(o => {
              const v = persona[k]; const d = PERSONA[k];
  ...
  {screen === "history" && (
    <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
      ...
      {history.length === 0 ? (
        ...
        <>
          <div style={{ background: C.surface, borderRadius: 12, padding: 16, marginBottom: 16, border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 12, color: C.textSub, marginBottom: 12, fontWeight: 600 }}>スコア推移（直近10回）</div>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 80 }}>
              {history.slice(-10).map((h, i) => {
                const ht = (h.score / 100) * 72;
                return (
                  <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                    <div style={{ fontSize: 9, color: gradeColor(h.grade), fontWeight: 700 }}>{h.grade}</div>
                    <div style={{ width: "100%", height: ht, background: gradeColor(h.grade), borderRadius: "3px 3px 0 0", transition: "height 0.5s ease" }} />
                    <div style={{ fontSize: 8, color: C.textMuted, textAlign: "center" }}>{h.date.slice(5)}</div>
                  </div>
                );
              })}
            </div>
          ...
          {[...history].reverse().map((h, i) => (
            <div key={i} style={{ background: C.surface, borderRadius: 10, padding: "10px 14px", marginBottom: 8, border: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
          ...
          <button onClick={() => { if (window.confirm("履歴を全削除しますか？")) { setHistory([]); saveHistory([]); } }}
      ...
      <button onClick={() => setScreen(persona.industry ? "main" : "persona")}
        style={{ width: "100%", padding: "12px 0", marginTop: 12, background: `linear-gradient(135deg,${C.gold},${C.goldDark})`, border: "none", borderRadius: 12, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
        ← 戻る
      </button>
    </div>
  )}
